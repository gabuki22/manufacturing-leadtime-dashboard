...
def render_mfg_report(load):
    ...
    # ── 3. 분석 방법론 · 데이터 아키텍처 ────────────────────────────
    st.markdown("## 3. 분석 방법론 · 데이터 아키텍처")
    _flow([("BigQuery", "원천 53테이블"), ("make_snapshot.py", "쿼리 재계산"),
           ("스냅샷 CSV", "data/ 배포 안전"), ("대시보드", "Streamlit"), ("이 리포트", "원본 재계산")])
    st.markdown(
        "- **데이터**: master · worklog 6공정 · jig_master · daily_output · 수주진단 · measurement · defect (BigQuery `wiki_manufacturing`)\n"
        "- **기법**: 리드타임 역산 → 병목 진단(TOC) → 백워드 스케줄링 → 유의성 검정(p-value) → 교란변수 층화(심슨) → 다축 세그(률 기준)\n"
        "- **신뢰 원칙**: 표본 30 미만 참고용 · 상관 ≠ 인과 · 값은 원본에서 재계산(하드코딩 없음)"
    )
