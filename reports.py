...
def render_mfg_report(load):
    ...
    st.success(
        f"1. **도장이 병목이다** — 일 **{bcap:,}개/일**로 최소 공정. 지그 실사가 장부보다 **{jig_gap}% 적어**"
        f"({jig_real:,} vs {jig_book:,}) 병목의 실제 원인으로 **보인다**. (실측 확인 — 원인 해석은 상관 기반)\n"
        f"2. **수주 {n_fail}건은 접수 시점에 이미 불가능** — 전체 {n_total}건 중 확보기간 < 필요 리드타임. (확인됨)\n"
        f"3. **잔업은 상황 문제이지 사람 문제가 아니다** — 잔업일 {ot_over}% vs 정상 {ot_norm}%(p<0.0001), "
        f"작업자별 무관(p=0.60), **물량 교란도 기각**(잔업일 물량 동일·3분위 층화 후에도 유지). "
        f"단 **대물량×잔업 조합이 {t3y}%로 최악**. (확인됨)\n"
        f"4. **불량의 진짜 조준점은 '블랙 도장 조건'** — 고객사는 색상의 대리변수(심슨 역전), 블랙 불량의 "
        f"3대 원인(흐름·미인쇄·색상편차)이 {top3_sum}%를 차지하며 전부 **도장조건·설비·조색 소관**이다. "
        f"Cpk는 실불량 예측 불가(r={r_cpk:+.2f}). (확인됨 — 층화·빈도 분석)"
    )
